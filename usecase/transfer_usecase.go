...
func (transferUseCase *transferUseCaseImpl) InsertTransfer(transfer *model.TransferModel, ctx *gin.Context) error {
	var wallet []float64
	senderExistData, err := transferUseCase.usrRepo.GetUserById(transfer.Send_id)
	if err != nil {
	...
	receiveExistData, err := transferUseCase.usrRepo.GetUserById(transfer.Receive_id)
	if err != nil {
		return fmt.Errorf("transferUsecaseImpl.InsertTransfer() 1: %w", err)
	}
...
func NewTransferUseCase(transferRepo repository.TransferRepo, usrRepo repository.UserRepo, bankRepo repository.BankRepo) TransferUseCase {
