...
func (mercRepo *merchantRepoImpl) InsertMerchant(merc *model.MerchantRequestModel) error {
	tx, err := mercRepo.db.Begin()
	if err != nil {
...
func (mercRepo *merchantRepoImpl) GetMerchantById(id string) (*model.MerchantModel, error) {
	qry := utils.GET_MERC_ID
	merc := &model.MerchantModel{}
	err := mercRepo.db.QueryRow(qry, id).Scan(&merc.ID, &merc.User_id, &merc.FullName, &merc.NoPhone, &merc.Email, &merc.Address)
	if err != nil {
...
func (mercRepo *merchantRepoImpl) GetMerchantByUserId(id string) (*model.MerchantModel, error) {
...
func (mercRepo *merchantRepoImpl) GetAllMerchant() ([]*model.MerchantModel, error) {
	qry := utils.GET_ALL_MERCHANT
	rows, err := mercRepo.db.Query(qry)
	if err != nil {
...
func (mercRepo *merchantRepoImpl) GetMerchantByName(name string) (*model.MerchantModel, error) {
	qry := utils.GET_MERC_NAME
	merc := &model.MerchantModel{}
	err := mercRepo.db.QueryRow(qry, name).Scan(&merc.ID, &merc.User_id, &merc.FullName, &merc.NoPhone, &merc.Email, &merc.Address)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func (mercRepo *merchantRepoImpl) EditMerchantById(merc model.MerchantModel) error {
	qry := utils.EDIT_MERC_ID
	_, err := mercRepo.db.Exec(qry, merc.FullName, merc.NoPhone, merc.Email, merc.Address, merc.ID)
	if err != nil {
